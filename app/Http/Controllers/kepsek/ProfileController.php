...
class ProfileController extends Controller
{
    ...
    public function editprofilePost (Request $request) {
        ...
        if ($data) {
            $result = DB::update('UPDATE kepala_sekolahs set Nama_Kepsek = ?, Jenjang_Pendidikan = ?, Jenis_Kelamin = ?, Tempat_Lahir = ?, Tanggal_Lahir = ?, TMT_Kerja = ?, Status = ? WHERE ID_Kepsek = 1', [$request->nama, $request->jenjang_pendidikan, $request->jenis_kelamin, $request->tempat_lahir, $request->tanggal_lahir, $request->tmt_kerja, $request->status]);
